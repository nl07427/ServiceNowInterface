...
namespace ServiceNow.Graph.Extensions
{
    ...
    public static class HttpRequestMessageExtensions
    {
        ...
        {
            httpRequestMessage.Headers.TryGetValues(Constants.Headers.FeatureFlag, out IEnumerable<string> flags);
        ...
        /// <summary>
        /// Gets a <see cref="ServiceNowRequestContext"/> from <see cref="HttpRequestMessage"/>
        /// </summary>
        /// <param name="httpRequestMessage">The <see cref="HttpRequestMessage"/> representation of the request.</param>
        /// <returns></returns>
        public static ServiceNowRequestContext GetRequestContext(this HttpRequestMessage httpRequestMessage)
        {
            ServiceNowRequestContext requestContext = new ServiceNowRequestContext();
            if (httpRequestMessage.Properties.TryGetValue(typeof(ServiceNowRequestContext).ToString(), out var requestContextObject))
            {
                requestContext = (ServiceNowRequestContext)requestContextObject;
            }
        ...
        public static T GetMiddlewareOption<T>(this HttpRequestMessage httpRequestMessage) where T : IMiddlewareOption
        {
            ...
            ServiceNowRequestContext requestContext = httpRequestMessage.GetRequestContext();
